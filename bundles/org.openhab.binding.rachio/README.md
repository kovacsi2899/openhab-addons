...
| `flexschedule` | Flex schedule rule with read-only status channels. |
| `basestation` | Smart Hose Timer Wi-Fi hub. |
| `valve` | Smart Hose Timer valve with manual start/stop and default runtime control. |
| `valveprogram` | Smart Hose Timer Program with schedule metadata, skip controls, and Program webhook state. |
...
- `stop` stops watering.
- Webhook registration succeeds in logs if callback is configured.
...
2. Click Add Manually at the end of the list.
3. Select the Rachio Binding.
4. Select the Rachio Cloud Connector Thing.
5. Enter at least the API key.
